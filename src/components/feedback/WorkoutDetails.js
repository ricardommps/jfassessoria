...
export function WorkoutDetails({
  ...
  const { mutate: sendComment } = useComments({
    invalidateQueries: ['history'], // ou qualquer query que precise atualizar
    onSuccess: () => commentsShow.onFalse(), // fecha o modal
  });

  // const hasUnreadUserComments = useMemo(() => {
  //   if (!itemFeedback?.comments?.length) return false;

  //   return itemFeedback.comments.some(
  //     (comment) => comment.isAdmin === false && comment.read === false,
  //   );
  // }, [itemFeedback?.comments]);
  const commentsCount = itemFeedback?.comments?.length ?? 0;
  const comments = itemFeedback?.comments || [];
    ...
    <>
          ...
          secondary={
            <>
              ...
              {commentsCount === 0 ? (
                ...
                <Typography variant="caption">{`Comentário do Aluno: ${itemFeedback?.comments[0]?.content}`}</Typography>
              )}
            </>
          }
        />
          ...
          {feedBackFormValue ? (
            <FormProvider
              ...
              onSubmit={handleSubmit((data) => {
                const commentId =
                  itemFeedback?.comments?.length > 0 ? itemFeedback.comments[0].id : undefined;
      ...
      {commentsShow.value && (
        <CommentsDialog
          ...
          onClose={commentsShow.onFalse}
          comments={itemFeedback.comments}
